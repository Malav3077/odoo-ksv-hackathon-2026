...
import { primaryNav, secondaryNav, utilityNav } from '@/config/navigation'

const LABELS = Object.fromEntries(
  [...primaryNav.flatMap(g => g.items), ...secondaryNav, ...utilityNav].map(item => [item.href, item.title])
)
...
function useBreadcrumbs(pathname) {
  return React.useMemo(() => {
    ...
    return segments.map((segment, index) => {
      ...
      return { label: LABELS[href] ?? titleize(segment), href, isLast: index === segments.length - 1 }
    })
  }, [pathname])
}
  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 shrink-0 items-center gap-2 border-b border-border/60 bg-background/80 px-3 backdrop-blur supports-[backdrop-filter]:bg-background/60 md:px-4">
      ...
      <div className="ml-auto flex items-center gap-1.5">
        ...
        <Button variant="ghost" size="icon" className="size-8 text-muted-foreground md:hidden" aria-label="Search">
          <Search className="size-4" />
        </Button>
