    ...
    <Sidebar collapsible="icon" variant="inset" {...props}>
            ...
            <SidebarMenuButton size="lg" asChild className="hover:bg-sidebar-accent">
              <Link href="/dashboard">
                ...
                <div className="grid flex-1 text-left leading-tight">
                  ...
                  <span className="truncate text-[11px] font-medium text-muted-foreground">{siteConfig.tagline}</span>
                </div>
              </Link>
            </SidebarMenuButton>
          </SidebarMenuItem>
      ...
      <SidebarFooter>
        <SidebarMenu>
          {utilityNav.map(item => (
            <SidebarMenuItem key={item.href}>
              <SidebarMenuButton asChild size="sm" className="text-muted-foreground" tooltip={item.title}>
                <Link href={item.href}>
                  {item.icon && <item.icon />}
                  <span>{item.title}</span>
                </Link>
